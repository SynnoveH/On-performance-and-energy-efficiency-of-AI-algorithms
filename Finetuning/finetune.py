...
# Split the training data into train and validation sets. Use (80/20 split)
...
def train_model(model_name, epochs=1, lr=1e-4, batch_size=64, train_loader=train_loader, val_loader=val_loader, dropout=0.0, weight_decay=0.00):
    # Train/fine-tune the model

    # Logging
    print(f"Starting training for model with set up: model name {model_name} | epochs {epochs} | lr {lr} | batch size {batch_size} | optimizer {optimizer_name} | dropout {dropout} | weight decay {weight_decay} | scheduler cosine_schedule_with_warmup |  seed {project_seed}")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("CUDA available:", torch.cuda.is_available())


    # Load the a ViT-model    
    ...
    model = load_vit(model_name, dropout=dropout)


    #=============================== Selecting optimizer=======================================================
    # Experimented with different optimizers in early stage but ended up using SGD
...
#==================================== Performance logging ===================================================
        # Calculate asverage loss and accuracy
        val_avg_loss = val_running_loss / len(val_loader)
        val_accuracy = val_correct / val_total

        # Calculate precison, recall, F1
    ...
    return model, avg_loss, accuracy, loss_list, accuracy_list, precision_list, val_loss_list, val_accuracy_list, val_precision_list, cm, val_recall_list, val_specificity_list, val_f1_list
...
with EmissionsTracker(project_name=f"{model_id}_training", experiment_id=exp_id, output_file=save_to_training, measure_power_secs=10, tracking_mode="process") as tracker:
    model, avg_loss, accuracy, loss_list, accuracy_list, precision_list, val_loss_list, val_accuracy_list, val_precision_list, val_cm, val_recall_list, val_specificity_list, val_f1_list = train_model(
        model_id, epochs=epochs, lr=lr, batch_size=batch_size, train_loader=train_loader, val_loader=val_loader, dropout=dropout, weight_decay=weight_decay)
