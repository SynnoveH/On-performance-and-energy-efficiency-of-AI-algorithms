...
# Modified: added support for Stanford Dogs dataset and CIFAR100 dataset
# Location in orginal repo is otdd/pytorch/datasets.py
...
DATASET_NORMALIZATION = {
    ...
    'STANFORDDOGS': ((0.485, 0.456, 0.406),(0.229, 0.224, 0.225))
}
...
def load_torchvision_data(dataname, valid_size=0.1, splits=None, shuffle=True,
    ...
    if data is None:
        if dataname == 'STANFORDDOGS':
            if datadir is None:
                datadir = DATA_DIR

            # Path to extracted Images folder
            data_path = os.path.join(datadir, 'stanford-dogs')
            ...
            # Add targets (REQUIRED for OTDD)
            train.targets = torch.tensor([label for _, label in train.samples])
            test.targets  = torch.tensor([label for _, label in test.samples])

            # Classes
            train.classes = test.classes = train.classes
            ...
            if dataname == 'EMNIST':
                ...
                # Ensure labels are 0..num_classes-1
                unique_labels = torch.unique(train.targets)
                label_map = {int(l): i for i, l in enumerate(unique_labels)}
                train.targets = torch.tensor([label_map[int(l)] for l in train.targets])
                test.targets  = torch.tensor([label_map[int(l)] for l in test.targets])
                
                # Set classes properly
                train.classes = test.classes = [str(i) for i in range(len(unique_labels))]
            else:
                train = DATASET(datadir, train=True, download=download, transform=train_transform)
                test = DATASET(datadir, train=False, download=download, transform=valid_transform)
    else:
